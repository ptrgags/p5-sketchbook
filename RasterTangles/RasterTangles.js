...
function preload() {
    for (let i = 0; i < LOW_MASK_COUNT; i++) {
        low_masks[i] = loadImage(`masks/low-${i}.png`);
    }

    for (let i = 0; i < MED_MASK_COUNT; i++) {
        med_masks[i] = loadImage(`masks/med-${i}.png`);
    }

    for (let i = 0; i < FINE_MASK_COUNT; i++) {
        fine_masks[i] = loadImage(`masks/fine-${i}.png`);
    }
}
...
function select_many(choices, n) {
    const result = new Array(n);
    for (let i = 0; i < n; i++) {
        result[i] = random(choices);
    }

    return result;
}

function raster_tangle(root_mask, first_choices, second_choices) {
    img = createImage(MASK_WIDTH, MASK_HEIGHT);
    img.loadPixels();
    ...
    for (let i = 0; i < pixel_count; i++) {
        ...
        const pixel_color = color(PALETTE[level2 % PALETTE.length]);
        
        // img.set() is likely redundant, just operate on the pixels array
        img.pixels[4 * i + 0] = red(pixel_color);
        img.pixels[4 * i + 1] = green(pixel_color);
        img.pixels[4 * i + 2] = blue(pixel_color);
...
function setup() {
    noLoop();
    createCanvas(3 * MASK_WIDTH, 3 * MASK_HEIGHT);
    ...
    const root_mask = random(low_masks);
    ...
    // all pixels.
    const first_choices = select_many(med_masks, 8);
    const second_choices = select_many(fine_masks, 8 * 8);

    const tangle = raster_tangle(root_mask, first_choices, second_choices);
    noSmooth();
    pixelDensity(1);
    image(tangle, 0, 0, width, height);
}
