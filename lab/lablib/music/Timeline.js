 ...
 * @returns
 */
export function intervals_at_time(timeline, time, result) {
  if (time < 0) {
    return;
  }
  ...
  if (timeline instanceof Sequential) {
    let offset = 0.0;
    for (const child of timeline.children) {
      if (time >= offset + child.duration.real) {
        intervals_at_time(child, time - offset, result);
        break;
      }

      offset += child.duration.real;
    }
    return;
  }
  ...
  if (timeline instanceof Parallel) {
    for (const child of timeline.children) {
      intervals_at_time(child, time, result);
    }
    return;
  }

  if (time <= timeline.duration.real) {
